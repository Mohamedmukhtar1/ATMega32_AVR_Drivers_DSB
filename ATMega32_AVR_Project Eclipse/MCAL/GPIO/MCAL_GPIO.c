...
void MGPIO_InitPinMode(u8 GPIOx, u8 GPIO_PIN_x, u8 Pin_Config_IO) {
	switch (GPIOx) {
	case GPIOA :
		REG_DDRA |= (Pin_Config_IO << (GPIO_PIN_x));
		break;
	case GPIOB :
		REG_DDRB |= (Pin_Config_IO << (GPIO_PIN_x));
		break;
	case GPIOC :
		REG_DDRC |= (Pin_Config_IO << (GPIO_PIN_x));
		break;
	case GPIOD :
		REG_DDRD |= (Pin_Config_IO << (GPIO_PIN_x));
		break;
	default: /*ERROR*/
		break;
	}
...
void MGPIO_WritePin(u8 GPIOx, u8 GPIO_PIN_x, u8 Pin_OP_State_HiLw) {
	...
	case GPIO_HIGH :
		...
		case GPIOA :
			REG_PORTA |= (1U << (GPIO_PIN_x));
			break;
		case GPIOB :
			REG_PORTB |= (1U << (GPIO_PIN_x));
			break;
		case GPIOC :
			REG_PORTC |= (1U << (GPIO_PIN_x));
			break;
		case GPIOD :
			REG_PORTD |= (1U << (GPIO_PIN_x));
			break;
		default: /*ERROR*/
			break;
		}
...
void MGPIO_WritePort(u8 GPIOx, u8 GPIO_PortValue) {
	...
	//GIVE_BIT(GPIOA_SPTR->GPIO_MODER.RegisterAccess, 2U* PinNum, (u32)DirectionMode) ; break;
	case GPIOA :
		REG_PORTA = (GPIO_PortValue);
		break;
	case GPIOB :
		REG_PORTB = (GPIO_PortValue);
		break;
	case GPIOC :
		REG_PORTC = (GPIO_PortValue);
		break;
	case GPIOD :
		REG_PORTD = (GPIO_PortValue);
		break;
	default: /*ERROR*/
		break;
	}
